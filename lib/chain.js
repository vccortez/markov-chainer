...
const { internal, randomElement, stateSpace } = require('./util')
...
class Chain {
  ...
  static seed (run, { model, tokenMap, initialState, order } = {}) {
    ...
    for (let i = 0; i < run.length + 1; ++i) {
      ...
      if (!model.has(state)) {
        model.set(state, [stateSpace(), stateSpace()])
      }
